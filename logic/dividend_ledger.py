...
def estimate_monthly_dividends(
    ...
    """Estimate calendar-month dividends from trailing per-share dividends.

    The output is intentionally labelled as an estimate, not a confirmed
    dividend schedule. Historical per-share payouts are grouped into fixed
    numeric months 1 through 12 so the chart never falls back to English
    datetime month labels. Missing dividend histories produce 0 estimates.
    """
    ...
    for row in holdings.to_dict("records"):
        ...
        for idx, month in enumerate(months):
            per_share = to_float(monthly.get(month), 0.0)
